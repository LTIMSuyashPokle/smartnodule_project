...
#         # Return dictionary with None values instead of empty dict
#         return {
#             'metrics': None,
#             'alerts': None,
#             'analytics': None,
...
# Keep all your existing UI functions (display_prediction_results, etc.)
...
# def display_gradcam_explanation(explanation, original_image):
#     """Display Grad-CAM explanation - FIXED VERSION"""
...
#         # FIXED: Create heatmap using matplotlib instead of st.image with cmap
#         # Create heatmap visualization using matplotlib
#         fig_heatmap, ax_heatmap = plt.subplots(1, 1, figsize=(6, 6))
#         im = ax_heatmap.imshow(explanation_resized, cmap='jet')
#         ax_heatmap.axis('off')
#         ax_heatmap.set_title('Attention Heatmap', fontsize=14, fontweight='bold')
...
#         import io
#         buf = io.BytesIO()
#         fig_heatmap.savefig(buf, format='png', bbox_inches='tight', dpi=150)
#         buf.seek(0)
#         heatmap_image = Image.open(buf)
#         plt.close(fig_heatmap)  # Clean up
...
#         # Create heatmap as RGB (using jet colormap)
#         heatmap_rgb = plt.cm.jet(explanation_resized)[:, :, :3]  # Remove alpha channel

#         # Create overlay
#         overlay = 0.6 * original_normalized + 0.4 * heatmap_rgb
...
#         with col1:
#             st.image(original_image, caption="Original Image", width='stretch')#use_container_width=True)
        
#         with col2:
#             # FIXED: Use the matplotlib-generated heatmap image instead of st.image with cmap
#             st.image(heatmap_image, caption="Attention Heatmap", width='stretch')#use_container_width=True)
        
#         with col3:
#             st.image(overlay, caption="Overlay",width='stretch')# use_container_width=True)
...
#         # Additional analysis
#         max_attention = np.max(explanation_resized)
#         mean_attention = np.mean(explanation_resized)
...
#         with col3:
#             st.metric("Focus Ratio", f"{(explanation_resized > 0.5).sum() / explanation_resized.size:.1%}")
...
#                 st.write(f"Explanation shape: {explanation.shape}")
#                 st.write(f"Explanation range: [{np.min(explanation):.3f}, {np.max(explanation):.3f}]")
#             if original_image is not None:
#                 st.image(original_image, caption="Original Image", width='stretch')#use_container_width=True)
...
└── [Module 4 directories]
    ...
    with tab1:
        ...
        #             # Extract confidence and uncertainty from results response
        #             confidence = results.get('confidence', 0.0)
        #             uncertainty_level = results.get('uncertainty_level', 'Unknown')
                    
        #             # Update performance metrics after successful inference
        #             update_prediction_metrics(
        #                 processing_time=processing_time,
        #                 confidence=confidence,
        #                 uncertainty_level=uncertainty_level,
